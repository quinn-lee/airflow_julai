...
# 退件数据
with DAG(
    dag_id='send_out_stock',
    default_args=default_args,
    schedule_interval='5 22 * * *',  # UTC时间每天22点5分跑
    ...
    # 上架数据，发邮件通知全部客户
    send_out_stock_daily = SSHOperator(
        task_id='send_out_stock_sh',
        command='send_out_stock_sh/send_out_stock.sh',
        ssh_hook=sshHook
    )
    ...
    email_task = EmailOperator(
        ...
        subject='{{ ds }}出库库存数据邮件发送已完成',
        html_content="""<h3>任务正常结束<h3>"""
    )

    send_out_stock_daily >> email_task
